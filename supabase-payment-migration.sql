...
ALTER TABLE experience_edition_requests
  ADD COLUMN IF NOT EXISTS payment_status TEXT DEFAULT 'NEACHITAT';
...
-- Update online_live_registrations: drop old check and add new one with all statuses
...
ALTER TABLE online_live_registrations
  ...
  CHECK (payment_status IN ('NEACHITAT', 'PLATA IN ASTEPTARE', 'ACHITAT', 'ANULAT', 'RAMBURSAT'));
